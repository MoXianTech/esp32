...
void lwip_thread(void *pvparams)
{
    ...
    while(1)
    {
        ...
        if (((timer_count % 2) == 0) && ((upload_flag & 0x03) == 0x03))
        {
        ...
        memset(lcd_buffer_display, 0x00, 64);
        sprintf(lcd_buffer_display, "pos_z-> %.2f fps %lld", queue_pressure.pressure_device_info.pos_z,
                                                             queue_pressure.port_frame_count - last_count);
        last_count = queue_pressure.port_frame_count;

        display_color = BLUE;
        lcd_show_string(50, 150, 240, 16, 16, lcd_buffer_display, display_color);
#endif
