...
#define LWIP_THREAD_STK_SIZE  20 * 1024              /* 任务堆栈大小 */
...
#define RECORD_THREAD_PRIO      3              /* 任务优先级 */
#define RECORD_THREAD_STK_SIZE  20 * 1024              /* 任务堆栈大小 */
TaskHandle_t RECORD_THREAD_Handler;  /* 任务句柄 */
...
void thraed_work_init(void)
{
    ...
    xTaskCreatePinnedToCore((TaskFunction_t )record_thread,
                            (const char *)  "RECORD_THREAD",
                            (uint16_t)       RECORD_THREAD_STK_SIZE,
                            (void *)         &thread_pvparam,
                            (UBaseType_t)    RECORD_THREAD_PRIO,
                            (TaskHandle_t *) &RECORD_THREAD_Handler,
                            (BaseType_t)     0);
